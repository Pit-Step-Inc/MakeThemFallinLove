# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Catherine 'sadness' (she wells up and a tear falls).

Ten drawings.  sadness_prep.py freezes everything onto the dry medoid (drawing
2) and patches in two things: the mouth on every frame, and - on the four
drawings that have one - the tear, lifted by its colour rather than by an
outline (see the prep for why neither of the other patch kinds works here).

So the clip opens on a plain sad face and the tear arrives during it:

  2 9 4 3 1 0   dry; the mouth trembles
  5             a bead at the outer lash        15 px
  8             the bead swells                 45 px
  6             it runs down the cheek         111 px
  7             the drop has come away          62 px, and the mouth sobs

then back to the dry drawings - the tear has fallen and is gone, which is what
lets the clip loop without the tear snapping off mid-cheek.  The drop is held
eight frames: it is the beat the clip is built around.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Catherine/frames_sadness/cs_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/cs_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "CatherineSad_"

TL = [
    (2, 4),                                     # a sad face, dry
    (9, 2), (4, 2), (3, 2), (1, 2), (0, 2),     # the mouth trembles
    (5, 4),                                     # a bead at the lash
    (8, 4),                                     # it swells
    (6, 4),                                     # it runs down the cheek
    (7, 8),                                     # the drop falls - held
    (0, 2), (3, 2),                             # gone; the frown remains
    (9, 2), (4, 2), (1, 2),
    (2, 4),
]
...
sc.frame_set(1)
print("catherine sadness: %d drawings, %d cuts, %d frames (%.2fs)"
