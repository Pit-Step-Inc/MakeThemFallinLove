# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Catherine 'anger' (a glare that becomes a shout).

Ten drawings; anger_prep.py froze everything outside one patch on the mouth, so
all ten frames are pixel identical outside 2584 px, and the base is the medoid
(drawing 9).  Only the mouth moves - the brows do sharpen on the two shouts but
they cannot be patched on this set (see anger_prep.py).

Sorted by how far the mouth is open the ten are

  8 0 9 7 1   a flat frown, each drawn a little differently
  2 6         gritted - the teeth show
  3 4         open
  5           the shout, teeth and tongue

**The teeth are kept, and the timeline is built around them.**  In joy the
teeth flashed, because two drawings of nine had them at an opening where the
others did not; here they are the expression, so 2 and 6 play as one block and
5 is reached through 3 and 4 - the teeth appear twice, each time as a held
beat, never alternating with a teeth-free mouth of the same opening.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Catherine/frames_anger/ca_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/ca_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "CatherineAnger_"

TL = [
    (9, 4),                                     # a flat glare
    (8, 2), (0, 2), (7, 2), (1, 2),             # the jaw sets
    (2, 3), (6, 3),                             # gritted teeth - one block
    (1, 2), (7, 2),                             # and released
    (3, 2), (4, 2),                             # the mouth opens
    (5, 5),                                     # the shout
    (4, 2), (3, 2),                             # closing
    (1, 2), (0, 2), (8, 2),
    (9, 4),                                     # back to the glare
]
...
sc.frame_set(1)
print("catherine anger: %d drawings, %d cuts, %d frames (%.2fs)"
