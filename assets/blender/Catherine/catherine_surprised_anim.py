# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Catherine 'surprised'.

Nine drawings.  surprised_prep.py freezes everything onto **drawing 4** - the
widest eyes and the highest brows of the nine - and patches in each drawing's
own mouth.  So the whole clip stares, wide-eyed, and the mouth does the acting;
the same trade Martin's surprised02 makes.  (An eye patch was built and measured
here and then taken back out - see surprised_prep.py's docstring for how to
restore it.)
...
A surprise has no anticipation, so the timeline does not build: two cuts to
establish, then straight to the full gasp, a waver on it, and only then down
through the "o" and back to the line.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Catherine/frames_surprised/cu_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/cu_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "CatherineSurprised_"

TL = [
    (8, 4), (0, 2),                             # before
    (4, 5),                                     # the gasp - straight there
    (3, 2), (4, 3), (3, 2), (4, 2),             # it wavers
    (2, 2), (5, 2), (6, 2),                     # coming down
    (1, 2), (7, 2), (0, 2),
    (8, 5),                                     # settled
]
...
sc.frame_set(1)
print("catherine surprised: %d drawings, %d cuts, %d frames (%.2fs)"
