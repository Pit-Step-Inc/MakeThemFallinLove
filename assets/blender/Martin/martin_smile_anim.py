# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation from the Martin expression sheet.

One plane per drawing, only one visible at a time (booleans keyframe as hard
cuts).  The sheet runs neutral -> smile -> laugh, so the timeline builds the
smile, holds a laugh, then settles back for a seamless loop.
"""
import bpy, os

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Martin/frames_smile/m_%02d.png"
NF = 6
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24

# (frame index, frames held) - 0 neutral ... 5 laughing
TL = [(0, 10),
      (1, 3), (2, 2), (3, 2),
...
sc.frame_set(1)
print("martin: %d drawings, %d frames (%.2fs), %d cuts"
