# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Martin 'anger' (scowl, then a shout).

anger_prep.py has already fitted the ten drawings onto one another on the
glasses/eye band and frozen everything but the mouth, so all ten frames are
pixel identical outside a 2248 px patch on the cheek: the picture does not
boil, and the only thing that moves is what he is doing with his mouth.
...
authored.  Sorted by how far the mouth is open they are
  ...
  9 0 8 2 1   shut - a flat scowl line, each drawn a little differently
  3 7 4       teeth showing, narrow to wide
  6           gritted, wide
  5           the full shout

which gives: hold the scowl and let the jaw clench a couple of times, wind up
over four frames, shout, flap the shout twice, then close back down to the
scowl and hold.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Martin/frames_anger/a_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/a_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "MartinAnger_"
...
TL = [
    (0, 8),                                     # scowl, held
    (8, 5), (2, 5), (0, 6), (9, 4),             # the jaw clenching
    (3, 2), (7, 2), (4, 2), (6, 2),             # winds up
    (5, 5),                                     # the shout
    (6, 2), (5, 4), (6, 2), (5, 4),             # yelling
    (4, 2), (6, 2), (5, 3),
    (7, 2), (3, 2), (1, 3),                     # closing down
    (0, 10),                                    # back to the scowl
]
...
sc.frame_set(1)
print("anger: %d drawings, %d cuts, %d frames (%.2fs)"
