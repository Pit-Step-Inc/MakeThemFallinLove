# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Martin 'surprised02' (caught off guard).

Named 02 because martin_surprised already exists - the six drawing set from the
first pass.  This is a separate, longer clip, not a replacement.

surprised02_prep.py has frozen everything outside one patch on the mouth, so
all ten frames are pixel identical outside 1428 px.  The base is drawing 4
rather than the medoid, because drawing 4 is the one with the widest eyes and
the eyes cannot be patched (its glasses are redrawn with them - see the prep).
So the stare is held for the whole clip and the mouth does the acting.
...
A surprise has no anticipation, so the timeline does not build: it sits on the
small mouth, cuts straight to the widest, wavers there, and only then comes
down through every stage in order.

Every cut but the pop and the two ends is held 2 frames, which at 24fps is 12
drawings a second - the floor for cutting a mouth before it reads as a flicker.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Martin/frames_surprised02/s2_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/s2_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "MartinSurprised02_"

TL = [
    (9, 4),                                     # before
    (5, 5),                                     # the pop - straight to the widest
    (4, 2), (5, 3),                             # the gasp wavers
    (4, 2), (2, 2), (3, 2), (1, 2),             # coming down
    (6, 2), (7, 2), (0, 2), (8, 2),
    (9, 5),                                     # settled
]
...
sc.frame_set(1)
print("surprised02: %d drawings, %d cuts, %d frames (%.2fs)"
