# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Martin 'talk03' (talking, hands in his pockets).

Named talk03 because martin_talk01 already exists - the six drawing set from
the first pass.  This continues the talk / talk01 / talk02 / talkhand numbering
rather than replacing anything.

talk03_prep.py has frozen everything outside one patch on the mouth, so all ten
frames are pixel identical outside 1446 px.  Unlike the emotion clips there is
nothing else to animate here and nothing to pin: the base is the medoid
(drawing 3).
...
Ten mouths is enough for real lip sync, so the timeline is three phrases rather
than one swell: each runs through a mix of shapes at 2 frames apiece (12
drawings a second, the tempo the README calls 早口) and lands on a shut mouth
for the word break.  For a calmer delivery, widen every hold in TL from 2 to 3
(8 drawings a second) - the same knob the martin_talk JSON documents.
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Martin/frames_talk03/t3_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/t3_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "MartinTalk03_"

TL = [
    (0, 2), (7, 2), (2, 2), (5, 2), (1, 2), (4, 2), (6, 2), (9, 3),   # phrase 1
...
sc.frame_set(1)
print("talk03: %d drawings, %d cuts, %d frames (%.2fs)"
