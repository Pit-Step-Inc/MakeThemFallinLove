# -*- coding: utf-8 -*-
"""Frame-by-frame 2D animation: Martin 'happy' (cheer).

Unlike talk / talkhand nothing is frozen here - the fifteen drawings really are
a pose progression (gesture, fists up, arms thrown wide), so the body has to
move.  happy_prep.py only fixes the placement, locking the head, which is the
one rigid landmark; the arms are the animation.

The drawings are not in playing order on the sheet, so the timeline is authored:
wind up, two fist pumps, arms wide, then settle back.

  0-3  gesture, hand out (0 = shut mouth)   4 arms at the sides
  5-9  fists rising                        10 both fists high
 11    arms up in a V                      12-13 arms thrown wide
 14    arms down, relaxed
"""
import bpy, os, math, glob

SRC = r"E:/001_MAKETHEMFALLinlove素材庫/blender/develop/Martin/frames_happy/p_%02d.png"
NF = len(glob.glob(os.path.dirname(SRC) + "/p_*.png"))
PLANE_H = 4.40
PLANE_Y = 0.50
FPS = 24
PREFIX = "MartinHappy_"
...
TL = [
    (14, 3),                                        # neutral
    (4, 2), (5, 2), (6, 2), (7, 2),                 # wind up
    (8, 2), (9, 2), (10, 3),                        # fists up
    (9, 2), (10, 2), (9, 2), (10, 3),               # two pumps
    (11, 3), (12, 3), (13, 3), (12, 3),             # arms thrown wide
    (11, 2), (10, 2), (8, 2), (6, 2), (4, 2), (14, 4),   # settle back
]
...
sc.frame_set(1)
print("happy: %d drawings, %d cuts, %d frames (%.2fs)"
